...
- **曝光 Stops**：按摄影里的 EV stops 思路处理，`-1 stop` 约等于把光量乘以 `0.5`，`+1 stop` 约等于乘以 `2`。它会改变整条输入输出曲线，不是简单给每个像素加减一个亮度值。
- **亮度**：通常指线性加减或显示器背光亮度。本工具不直接控制显示器背光，也没有单独做简单亮度偏移。
- **Gamma**：主要改变中间调。小于 `1` 会压暗中间调，大于 `1` 会提亮中间调；黑点和白点相对更稳定。
- **曲线**：最适合处理“室外太亮但室内刚好”的情况。把曲线右侧往下拉，可以重点压暗高光；保持左侧接近原位，可以少影响暗部和室内。

实际实时输出受 Windows `SetDeviceGammaRamp` 限制。为了避免部分显卡驱动拒绝写入，程序会保留一个驱动安全白场下限；如果需要比实时 Gamma Ramp 更极端的效果，优先在游戏内亮度/曝光或显卡滤镜里进一步调整。
...
- 已开启 HDR 或 Advanced Color。
- 游戏使用独占全屏或受保护输出。
- 显卡驱动、显示器软件或系统夜间模式覆盖了 Gamma Ramp。
- 曲线把白场压得过低，显卡驱动拒绝 `SetDeviceGammaRamp` 写入。
- 游戏本身绕过 Windows 色彩管理。

建议先关闭 HDR，用无边框窗口或窗口模式测试。如果状态栏提示 `SetDeviceGammaRamp 写入失败`，先降低压暗幅度，或把曲线右上端点提高一些。
